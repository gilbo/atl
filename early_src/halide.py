...
for C,ctyp,htyp in [
    ...
    ('PL','struct hw_pipeline_t', 'Halide::Pipeline'),
    ]:
        HWGen.c_decl(f"{ctyp} {{"
        f"    uint64_t id;"
        f"}};")
...
# PIPELINE
class hw_pipeline_t(ctypes.Structure):
    _c_type_str = "struct hw_pipeline_t"
    _fields_ = [ ('id', ctypes.c_ulonglong) ]
...
HWGen.fun("hwrap_delete_pipeline",[('pl_handle',hw_pipeline_t)],'v',"""
    delete _from_PL(pl_handle);""")
...
def _install_destructors(mod):
    ...
    def _hw_param_t_del(self):
        if self.id != 0:
            ...
            self.id = 0
    def _hw_pipeline_t_del(self):
        if self.id != 0:
            mod.hwrap_delete_pipeline(self)
            self.id = 0
    ...
    hw_pipeline_t.__del__ = _hw_pipeline_t_del
...
# PIPELINE
HWGen.fun("hwrap_new_pipeline",
    [('n','i32'),
     ('funcs',ctypes.POINTER(hw_func_t)),],
    hw_pipeline_t,"""
    std::vector< Halide::Func > fs;
    for(int k=0; k<n; k++)
        fs.push_back( *_from_F(funcs[k]) );
    return _to_PL(new Halide::Pipeline(fs));""")
...
HWGen.fun("hwrap_realize_pipeline",
    [('self',hw_pipeline_t),
     ('n','i32'),
     ('buffers',ctypes.POINTER(halide_buffer_t))],'v',"""
    std::vector< Halide::Buffer<> > bufs;
    for(int k=0; k<n; k++)
        bufs.push_back(Halide::Buffer<>(buffers[k]));
...
HWGen.fun("hwrap_autoschedule_pipeline",
    ...
    auto s = _from_PL(self)->auto_schedule(
                    Halide::get_host_target(),
                    Halide::MachineParams::generic());""")
